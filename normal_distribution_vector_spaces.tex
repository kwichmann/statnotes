...
Some notes are in order, before we procees. For now "$\Sigma^{\frac{1}{2}}$" should simply be considered a matrix of dimensions $n\times p$, not necessarily a square root of anything (we will get back to that later). It plays the roles that $\sigma$ played for the univariate case\footnote{Hence, simply calling it $\Sigma$ would be more be clearer, but such is the tradition notation.}, and hence we might expect that special care needs to be taken when it is "zero", whatever that might signify here.
