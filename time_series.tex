...
\subsection{AR($p$)}
Using the decomposition of the characteristisc polynomial for equation \ref{rewritten_characteristic}, the model is:
\begin{equation}
...
\end{equation}
This is known as the \textit{Koyck transformation} - we have turned an AR(1) model into a MA($\infty$) model! Of course, we could have gotten the same result by repeatedly applying equation \ref{AR1}
