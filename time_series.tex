...
\begin{equation}
\label{AR_polynomial}
\underbrace{\left(1-\sum_{i=1}^p\theta_i L^i\right)}_{\phi(L)}X_t=\varepsilon_t
\end{equation}
...
\begin{equation}
\phi(z)=-\theta_p z^p-\theta_{p-1}z^{p-1}-\ldots -\theta_1 z+1
\end{equation}
...
\phi(z)=-\theta_p(z-z_1)(z-z_2)\cdots(z-z_p)
\end{equation}
According to Vieta's equations, we have:
\begin{equation}
z_1 z_2\cdots z_p=(-1)^p\frac{1}{-\theta_p}\Leftrightarrow -\theta_p=\frac{(-1)^p}{z_1 z_2\cdots z_p}=(-\phi_1)(-\phi_2)\cdots(-\phi_p)
\end{equation}
